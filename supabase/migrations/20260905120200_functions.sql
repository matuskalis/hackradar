-- Both functions run as SECURITY INVOKER (the default), so the caller's RLS
-- still applies on top of the explicit status filter.
...
begin
  ...
  -- 17 = distance_km; ordering by the output alias would be ambiguous here.
  order by 17 nulls last, h.start_at
  limit max_rows;
end;
$$;
...
-- Atomic counter for the request rate limiter. VOLATILE by default; the upsert
-- keeps concurrent requests from over-permitting.
...
revoke execute on function bump_rate_limit(text, timestamptz) from public;
